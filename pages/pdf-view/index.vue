    ...
    <div class="pdf-view-wrapper">
      <div class="action-wrapper">
        <div class="content" :class="[isCollapsed ? 'collapsed' : 'expand']">
          ...
          <div class="main-wrapper" v-show="!isCollapsed">
            <div
...
export default {
  ...
  methods: {
    getConversationList() {
      ...
      this.$request.getConversationList()
        .then((res) => {
    ...
    addConversation({ name, uuid }) {
      ...
      this.$request.createConversation(data)
        .then((res) => {
          // 如果没有登录就存入缓存
          if (!this.userInfo.phoneNumber) {
            const list = localStorage.getItem("pdf-conversation-list");
