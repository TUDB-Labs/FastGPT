...
export default {
  ...
  methods: {
    ...
    onShowShareModal() {
      ...
      this.$request.getShareInfo({ docUuid: this.pdfBaseInfo.docUUid })
        .then((res) => {
    ...
    onRest() {
      this.$confirm("确定要清空对话内容吗？", "提示", {
        ...
        .then(() => {
          this.$request.clearConversationDetails({ uuid: this.pdfBaseInfo.uuid }).then(() => {
    ...
    onDelete() {
      this.$confirm("确定要删除此PDF吗？", "提示", {
        ...
        .then(() => {
          ...
          //   return;
          // }
          this.$request.deleteConversationDetails({ uuid: this.pdfBaseInfo.uuid }).then(
            () => {
    ...
    onChangeName(ok) {
      ...
      this.$request.renameConversation({
